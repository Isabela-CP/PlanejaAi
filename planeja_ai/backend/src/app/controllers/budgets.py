...
def calculate_budget_spent(user_id, category_id, month_year, reset_day):
    ...
    spent = (
        db.session.query(func.coalesce(func.sum(Transaction.value), 0))
        .filter(
...
def list_budgets():
    ...
    if date_str:
        try:
            ...
            try:
                ...
                return (
                    jsonify(
                        {"error": "Formato de data inválido. Use AAAA-MM-DD ou AAAA-MM"}
                    ),
                    400,
                )
    else:
    ...
    budgets = Budget.query.filter_by(
        user_id=request.user_id, month_year=month_year
    ).all()

    result = []
    for b in budgets:
        b_dict = b.to_dict()
        b_dict["spent"] = calculate_budget_spent(
            request.user_id, b.category_id, b.month_year, b.reset_day
        )
...
def create_budget():
    ...
    if category.type != "transaction":
        return (
            jsonify({"error": "A categoria fornecida deve ser do tipo transaction"}),
            400,
        )

    # Validar se já existe orçamento para essa categoria no mês correspondente
    existing = Budget.query.filter_by(
        user_id=request.user_id, category_id=category_id, month_year=month_year
    ).first()
    if existing:
        return (
            jsonify(
                {
                    "error": "Já existe um orçamento definido para esta categoria no mês correspondente"
                }
            ),
            400,
        )
    ...
    b_dict["spent"] = calculate_budget_spent(
        request.user_id, category_id, month_year, reset_day_val
    )
...
def update_budget(budget_id):
    ...
    if limit_value is not None:
        try:
            ...
            if limit_val < 0:
                return (
                    jsonify({"error": "O limite deve ser maior ou igual a zero"}),
                    400,
                )
    ...
    b_dict["spent"] = calculate_budget_spent(
        request.user_id, budget.category_id, budget.month_year, budget.reset_day
    )
