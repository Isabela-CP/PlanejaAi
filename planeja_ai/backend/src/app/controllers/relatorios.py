...
def resumo():
    ...
    total_metas = (
        db.session.query(func.coalesce(func.sum(Goal.current_value), 0))
        .filter(Goal.user_id == request.user_id)
        .scalar()
    )
...
def por_categoria():
    ...
    for r in results:
        ...
        percentage = (
            round((amount_val / total_despesas) * 100, 2) if total_despesas > 0 else 0.0
        )
