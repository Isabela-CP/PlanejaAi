...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _exportReport(String format) async {
    ...
    try {
      ...
      if (format == 'csv') {
        ...
        csv.writeln(
            '${totalIncome.toStringAsFixed(2)};${totalExpenses.toStringAsFixed(2)};${netIncome.toStringAsFixed(2)}');
        csv.writeln('');
        ...
        for (var item in breakdown) {
          ...
          csv.writeln(
              '$cat;${val.toStringAsFixed(2)};${pct.toStringAsFixed(2)}%');
        }
        csv.writeln('');
        ...
        for (var item in evolution) {
          ...
          csv.writeln(
              '$mes;${rec.toStringAsFixed(2)};${des.toStringAsFixed(2)};${sal.toStringAsFixed(2)};${acu.toStringAsFixed(2)}');
        }
        ...
        final excelBytesList =
            await _generateExcelBytes(summary, breakdown, evolution);
  ...
  Future<Uint8List> _generatePdfBytes(
      Map summary, List rawBreakdown, List evolution) async {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (pw.Context context) {
        return [
          pw.Header(
              level: 0,
              child: pw.Text('Relatório Financeiro - Planeja.Ai',
                  style: pw.TextStyle(
                      fontSize: 24, fontWeight: pw.FontWeight.bold))),
          pw.SizedBox(height: 20),
          pw.Text(
              'Período: ${_formatDate.format(_startDate!)} a ${_formatDate.format(_endDate!)}'),
          pw.SizedBox(height: 20),
          pw.Text('Resumo',
              style:
          ...
          pw.TableHelper.fromTextArray(
            ...
            data: <List<String>>[
              ...
              <String>[
                _formatCurrency.format(totalIncome),
                _formatCurrency.format(totalExpenses),
                _formatCurrency.format(netIncome)
              ],
            ],
            headerStyle: pw.TextStyle(
                color: PdfColors.white, fontWeight: pw.FontWeight.bold),
            headerDecoration:
                const pw.BoxDecoration(color: PdfColor.fromInt(0xFFD4A017)),
          ...
          pw.TableHelper.fromTextArray(
            ...
            data: <List<String>>[
              ...
              ...rawBreakdown.map((item) => [
                    ...
                    _formatCurrency
                        .format((item['amount'] as num?)?.toDouble() ?? 0.0),
          ...
          pw.TableHelper.fromTextArray(
            ...
            data: <List<String>>[
              ...
              ...evolution.map((item) => [
                    ...
                    _formatCurrency
                        .format((item['income'] as num?)?.toDouble() ?? 0.0),
                    _formatCurrency
                        .format((item['expense'] as num?)?.toDouble() ?? 0.0),
                    _formatCurrency
                        .format((item['net'] as num?)?.toDouble() ?? 0.0),
                    _formatCurrency.format(
                        (item['cumulative'] as num?)?.toDouble() ?? 0.0),
                  ])
            ],
            headerStyle: pw.TextStyle(
                color: PdfColors.white, fontWeight: pw.FontWeight.bold),
  ...
  Future<List<int>> _generateExcelBytes(
    ...
    sheet.appendRow([
      TextCellValue('Data Inicial:'),
      TextCellValue(_formatDate.format(_startDate!))
    ]);
    sheet.appendRow([
      TextCellValue('Data Final:'),
      TextCellValue(_formatDate.format(_endDate!))
    ]);
    ...
    sheet.appendRow([
      ...
      TextCellValue('Despesas Totais'),
      TextCellValue('Renda Líquida')
    ]);
    sheet.appendRow([
    ...
    sheet.appendRow([
      TextCellValue('Mês'),
      TextCellValue('Receitas'),
      TextCellValue('Despesas'),
      TextCellValue('Saldo'),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        decoration:
                            const InputDecoration(hintText: 'Selecione'),
                      ),
                    ],
                  ),
                ),
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label:
                        Text(_isGenerating ? 'Gerando...' : 'Gerar Relatório'),
            ...
            Consumer<FinanceProvider>(
              ...
              final categoriesList = financeProvider.transactionCategories
                  .map((c) => c.name)
                  .toList();
                      ...
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          ...
                          if (index >= 0 && index < evolution.length) {
                            final monthStr =
                                evolution[index]['month'] as String? ?? '';
  ...
  Widget _buildReportResults(BuildContext context) {
    ...
    final transactionCount =
        (summary['quantidade_transacoes'] as num?)?.toDouble() ?? 0.0;
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                        '${(item['percentage'] as num).toStringAsFixed(1)}% do total',
                                        style: TextStyle(
                                          fontSize: 12,
                                          color: theme.colorScheme.onSurface
                                              .withOpacity(0.6),
  ...
  Widget _buildSummaryCard(
      String title, double value, IconData icon, Color iconColor, int delayMs,
      {bool isCount = false}) {
    return Card(
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
